  ...
  const handleBuy = async () => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        router.push(`/admin/dashboard/book-ticket/${params.id}/payment`);
      } catch (err) {
        console.error(err);
      } finally {
        setIsLoading(false);
      ...
      <div>
        ...
        {tickets?.length > 0 &&
          tickets.map((ticket, index) => (
            <div className="white-box" style={{ marginTop: "16px" }} key={index} onClick={() => router.push(`/admin/dashboard/book-ticket/${params.id}/payment`)}>
              <div>
                Ticket no : <b>{ticket?.ticketNo}</b>
              </div>
              <div>
              Status: {(ticket?.status === "paid" ||  "offline-paid") ? "confirmed" : "payment is due" }             </div>
